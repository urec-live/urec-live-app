...
export default function DMConversationScreen() {
  ...
  useEffect(() => {
    ...
    messagingAPI.getDMHistory(username).then((history) => {
      setMessages(history);
      setLoading(false);
    }).catch(() => setLoading(false));

    messagingAPI.markRead(username).catch(() => {});

    const unsubscribe = websocketService.subscribeToUserQueue((msg: ChatMessageResponse) => {
      const isRelevant =
  ...
  const handleSend = () => {
    ...
    setInput("");
    websocketService.sendDM(username, text);
  };
  ...
  return (
    <LinearGradient colors={["#ffffff", "#f5f5f5", "#ffffff"]} style={{ flex: 1 }}>
      ...
      <View style={styles.header}>
        ...
        <Pressable onPress={() => router.push(`/users/${username}` as any)}>
